...
VARS0="--bind-to core --map-by socket -x CUDA_VISIBLE_DEVICES=0,1,2,3" # -x NVSHMEM_SYMMETRIC_SIZE=10737418240"
VARS1="-x UCX_WARN_UNUSED_ENV_VARS=n  -x HCOLL_RCACHE=^ucs -x LD_LIBRARY_PATH=/home/jciesko/software/nvshmem_src_2.9.0-2/install/lib:$LD_LIBRARY_PATH"
...
for S in $(seq 1 21); do 
   for reps in $(seq 1 3); do
      mpirun --tag-output -np 2 $VARS0 $VARS1 $VARS2 -host $HOST  ./$BENCHMARK -N $SIZE -I $ITERS -M 2 | tee -a $FILENAME
   done
   let SIZE=$SIZE*2
done
