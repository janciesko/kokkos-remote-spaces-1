...
struct Access_LDC<
  ...
  void operator()(const UpdateTag_get &, typename team_policy_get_update_t::member_type team) const {
    Kokkos::single(Kokkos::PerTeam(team), [&]() {
     ...
     for(int i = 0; i<10; ++i) {
       double v1 = v_subview_remote(i);
       double v2 = v_tmp_subview_local(i);
       double v3 = v(i);
       double v4 = v_tmp(i);
       printf("v_device_pre: %f, %f, %f, %f\n",v1,v2,v3,v4);
      }
      ...
      for (int i = 0; i < iters; i++) {
        if (my_rank == 0) {
          ...
          Kokkos::parallel_for("block_transfer", team_policy_get_update_t(1, 1),
                               *this);
          Kokkos::fence();
          Kokkos::parallel_for("printf", Kokkos::RangePolicy(local_range.first, local_range.second), *this);
          Kokkos::parallel_for(
