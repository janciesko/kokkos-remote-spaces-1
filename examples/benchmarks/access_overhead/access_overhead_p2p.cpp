...
using team_policy_update_t = Kokkos::TeamPolicy<>;
    ...
    if (rma_op == RMA_GET) {
      ...
      if (my_rank == 0) {
        ...
        Kokkos::parallel_for(
            ...
            policy_check_get_t(local_range.first, local_range.second), *this);
        Kokkos::fence();
#endif
      }
    } else {
      ...
      if (my_rank == 1) {
        ...
        Kokkos::parallel_for(
            "access_overhead-check",
            policy_check_put_t(local_range.first, local_range.second), *this);
        Kokkos::fence();
#endif
      }
    }
...
struct Access_LDC<
  ...
  ViewType_t v, v_tmp;
  ViewType_t v_subview_remote, v_tmp_subview_local;
  ...
  void operator()(const UpdateTag &, const size_t i) const {
    v(i) += v_tmp_subview_local(i);
  }

  KOKKOS_FUNCTION
  void operator()(typename team_policy_update_t::member_type team) const {
    Kokkos::single(Kokkos::PerTeam(team), [&]() {
      auto local_range = Kokkos::Experimental::getRange(num_ranks * N, my_rank);
      auto remote_range =
          Kokkos::Experimental::getRange(num_ranks * N, other_rank);
      auto v_subview_remote_auto    = Kokkos::subview(v, remote_range);
      auto v_tmp_subview_local_auto = Kokkos::subview(v_tmp, local_range);
      printf("LRange: %li, %li\n", local_range.second, local_range.first);
      printf("RRange: %li, %li\n", remote_range.second, remote_range.first);
      Kokkos::Experimental::RemoteSpaces::local_deep_copy(
          v_tmp_subview_local_auto, v_subview_remote_auto);
    });
  }
  ...
  void run() {
    ...
    if (rma_op == RMA_GET) {
      for (int i = 0; i < iters; i++) {
        if (my_rank == 0) {
          time_a = timer.seconds();
          Kokkos::parallel_for("block_transfer", team_policy_update_t(1, 1),
                               *this);
          Kokkos::fence();
          Kokkos::parallel_for(
      ...
      for (int i = 0; i < iters; i++) {
        if (my_rank == 0) {
          ...
          /*change this to team policy update PUT*/
          Kokkos::parallel_for("block_transfer", team_policy_update_t(1, 1),
                               *this);
          Kokkos::fence();
          ...
          Kokkos::parallel_for(
              "access_overhead",
              policy_update_t(local_range.first, local_range.second), *this);
  ...
  void run() {
    ...
    if (my_rank == 0) {
      ...
      printf("access_overhead,%s,%lu,%lf,%lu,%lf,%lf,%lf\n",
             (modes[mode] + "-MPIIsCudaAware").c_str(), N, size, iters, time,
             gups, bw);
#else
      printf("access_overhead,%s,%lu,%lf,%lu,%lf,%lf,%lf\n",
             modes[mode].c_str(), N, size, iters, time, gups, bw);
